...
export default function RevenuePage() {
  ...
  const gross7 = series.slice(-7).reduce((a, s) => a + (s.gross || 0), 0)
  const gross30 = series.reduce((a, s) => a + (s.gross || 0), 0)
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Gross revenue from every Spins purchase — the total players pay. How it&apos;s split across pools lives on the Distribution page. Players never see this.
      </p>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        <Card label="Gross, all time" value={usd(d.gross_total)} accent="#c6ff2e" hint={`${d.purchases} purchases`} />
        <Card label="Last 7 days" value={usd(gross7)} />
        <Card label="Last 30 days" value={usd(gross30)} />
        <Card label="Avg / purchase" value={usd(d.purchases > 0 ? d.gross_total / d.purchases : 0)} />
      </div>

      <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
        <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-gray-500 mb-4">
          <TrendingUp className="w-4 h-4" /> Gross revenue, last 30 days
        </div>
          ...
          <ResponsiveContainer>
            <AreaChart data={series} margin={{ top: 4, right: 8, bottom: 0, left: -16 }}>
              ...
              <Tooltip contentStyle={{ background: "#111827", border: "1px solid #374151", borderRadius: 8, fontSize: 12 }}
                labelStyle={{ color: "#9ca3af" }} formatter={(v: any) => [usd(Number(v)), "Gross"]} />
              <Area type="monotone" dataKey="gross" stroke="#c6ff2e" strokeWidth={2} fill="url(#grossFill)" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
